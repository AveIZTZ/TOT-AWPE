function [z, delay] = derev_awpe_RLS_TOT_change(y, Para_AWPE)
...
ct          = Para_AWPE.ct;
%h12p         = [ct*ones(1,P*L2);zeros(L12-1,P*L2)];
%h11p         = [ct*ones(1,P*L2);zeros(L11-1,P*L2)];
%h2p         = [ct*ones(1,L2);zeros(L2-1,L2)];
h12p = zeros(L12,P*L2);
h11p = zeros(L11,P*L2);
%if (P*L2)<L12
%    h12p         = [ct*eye(P*L2);zeros(L12-P*L2,P*L2)];
%else
%    h12p         = horzcat(ct*eye(L12),zeros(L12,P*L2-L12));
%end
%if (P*L2)<L11
%    h11p         = [ct*eye(P*L2);zeros(L11-P*L2,P*L2)];
%else
%    h11p         = horzcat(ct*eye(L11),zeros(L11,P*L2-L11));
%end

%for n = 1:L2
%    if P<L12
%        h12p(:,(n-1)*P+1:n*P)         = [ct*eye(P);zeros(L12-P,P)];
%    else
%        h12p(:,(n-1)*P+1:n*P)         = horzcat(ct*eye(L12),zeros(L12,P-L12));
%    end
%    if P<L11
%        h11p(:,(n-1)*P+1:n*P)         = [ct*eye(P);zeros(L11-P,P)];
%    else
%        h11p(:,(n-1)*P+1:n*P)         = horzcat(ct*eye(L11),zeros(L11,P-L11));
%    end
%end
...
for nf = 1:nfrms
    ...
    for nbin = 1 :nfbands %winsize
        ...
        for k=1:L2
            ...
            xk_12_11((k-1)*L2+1:k*L2) = conj(y_trans2'*G12_11_l);%(L2L11L12L2)
        end
        
        
        % Compute the output signal
        s_out2        = Xr - h_matrix_2(:,nbin)'*xk_12_11;%L2L2
        conj_s_out2   = conj(s_out2);
        
        % calculate the PSD
        %lamuda2       = max(s_out2*conj_s_out2,lamuda_min);
        lamuda2       = s_out2*conj_s_out2;%1
        
        % update the weighted cross-/correlation matrixh_matrix_2
        Bn_2          = Cov_y2(:,:,nbin) * xk_12_11;%L2L2*1,L2^4
        An_2          = xk_12_11'*Cov_y2(:,:,nbin);%1*L2L2,L2^4
        An_d          = 1/max(alpha*lamuda2 + xk_12_11'*Bn_2, 1e-6);%1*1,L2^2+2
        Kf_2          = An_d.*Bn_2;%L2L2*1,L2^2
        
        Cov_y2(:,:,nbin)     = inv_alpha_rls.*(Cov_y2(:,:,nbin) - Kf_2*An_2);%L2L2*L2L2,2L2^4
        % update the filter
        h_matrix_2(:,nbin)    = h_matrix_2(:,nbin) + conj_s_out2.*Kf_2;%L2L2        
        %elapsed_time = toc;
        %time(nbin+(nf-1)*length(nfbands))=elapsed_time;
        ...
        %P*L2^2*L11+L2^2*L11*L12
        % Compute the output signal
        s_out12        = Xr - h_matrix_12(:,nbin)'*xk_2_11;%PL12L2
        conj_s_out12   = conj(s_out12);
        
        % calculate the PSD
        %lamuda12       = max(s_out12*conj_s_out12,lamuda_min);
        lamuda12       = s_out12*conj_s_out12;%1
        
        % update the weighted cross-/correlation matrix
        Bn_12          = Cov_y12(:,:,nbin) * xk_2_11;%PL12L2*1,P^2L12^2L2^2
        An_12          = xk_2_11'*Cov_y12(:,:,nbin);%1*PL12L2,P^2L12^2L2^2
        An_d          = 1/max(alpha*lamuda12 + xk_2_11'*Bn_12, 1e-6);%1*1,2+PL12L2
        Kf_12          = An_d.*Bn_12;%PL12L2*1
        
        Cov_y12(:,:,nbin)     = inv_alpha_rls.*(Cov_y12(:,:,nbin) - Kf_12*An_12);%2P^2L12^2L2^2
        % update the filter
        h_matrix_12(:,nbin)    = h_matrix_12(:,nbin) + conj_s_out12.*Kf_12;%PL12L2
