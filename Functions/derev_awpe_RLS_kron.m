function [z, delay] = derev_awpe_RLS_kron_change(y, Para_AWPE)
...
if (P)<L2
    ...
    h2p         = horzcat(ct*eye(L2),zeros(L2,P-L2));
end
%h2p         = [ct*ones(1,P);zeros(L2-1,P)];
for nbin= 1 :nfbands
...
for nf = 1:nfrms
    ...
    for nbin = 1 :nfbands %winsize
        ...
        Bn_1          = Cov_y1(:,:,nbin) * xk_1;%P2L22
        An_1          = xk_1'*Cov_y1(:,:,nbin);%P2L22
        An_d          = 1/max(alpha*lamuda1 + xk_1'*Bn_1, 1e-6);%PL2+1
        Kf_1          = An_d.*Bn_1;%PL2
        
        Cov_y1(:,:,nbin)     = inv_alpha_rls.*(Cov_y1(:,:,nbin) - Kf_1*An_1);%2P2L22
        % update the filter
